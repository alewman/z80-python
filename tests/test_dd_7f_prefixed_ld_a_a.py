"""Red-first witness for the DD-prefixed LD A,A (DD 7F) form.
...
``NotImplementedError``.  This file pins DD 7F with one direct deterministic
unit test: PC=0x2000, IX=0x4000, A=F=Q=0xA5, R=0x3E, WZ=0xBEEF.

When DD 7F LD A,A is implemented it must behave exactly like the plain 0x7F
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed LD A,A (DD 7F) has no handler in ``_execute_index`` yet, so
    ...
    try:
        ...
        pytest.fail(
            f"DD-prefixed LD A,A is not implemented (opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_7f_prefixed_ld_a_a() -> None:
    """DD 7F LD A,A: A/IX/F/WZ unchanged, PC=0x2002, R=0x40, Q=0, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0xA5
    ...
    tstates = _run(cpu, bytes([0xDD, 0x7F]))

    assert cpu.a == 0xA5  # LD A,A copies A into A, so A is unchanged
    ...
    assert int(cpu.f) == 0xA5  # LD A,A leaves F untouched
