"""Red-first witness for the FD-prefixed SUB A,B (FD 90) form.

The DD/FD index-prefix family dispatches through ``_execute_index``, which
handles the indexed load/ALU/POP/PUSH/JP sub-opcodes -- but sub-opcode 0x90
(the SUB A,B form of the 0x80-0xBF ALU block) has no handler yet, so it
raises ``NotImplementedError``.  This file pins FD 90 with one direct
deterministic unit test: PC=0x2000, IY=0x5000, A=0x00, B=0x01, R=0x3E,
F=Q=0x00, WZ=0xBEEF.

When FD 90 SUB A,B is implemented it must behave exactly like the plain 0x90
SUB A,B (the prefix is ignored because neither operand is H/L or (HL)): A
receives 0x00 - 0x01 = 0xFF with S=1 Y=1 H=1 X=1 N=1 C=1 (F=0xBB), B and the
...
ignored prefix fetch plus 4 for the base SUB A,B).
"""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed SUB A,B (FD 90) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed SUB A,B is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_90_prefixed_sub_a_b() -> None:
    """FD 90 SUB A,B: A=0xFF, F=Q=0xBB, B/IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0x00
    ...
    tstates = _run(cpu, bytes([0xFD, 0x90]))
