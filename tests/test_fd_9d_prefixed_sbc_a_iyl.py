"""Red-first witness for the FD-prefixed SBC A,IYL (FD 9D) form.
...
(the SBC A,IYL form: the L slot of the 0x80-0xBF ALU block remapped by the
FD prefix to the low byte of IY) has no handler yet, so it raises
``NotImplementedError``.  This file pins FD 9D with one direct deterministic
unit test: PC=0x2000, IY=0x0201 (IYL=0x01 is the source, IYH=0x02 must be
ignored), A=0x00, R=0x3E, F=Q=0x01 (carry in set), WZ=0xBEEF.

When FD 9D SBC A,IYL is implemented it must behave exactly like SBC A,L with
the L slot replaced by the low byte of IY: A receives 0x00 - 0x01 - carry 1
= 0xFE with S=1 Y=1 H=1 X=1 N=1 C=1 (F=0xBB), the full IY register stays
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed SBC A,IYL (FD 9D) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed SBC A,IYL is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_9d_prefixed_sbc_a_iyl() -> None:
    """FD 9D SBC A,IYL: A=0xFE, F=Q=0xBB, IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x0201  # IYL=0x01 is the source; IYH=0x02 must be ignored
    cpu.a = 0x00
    cpu.r = 0x3E
