"""Red-first witness for the DD-prefixed ADC A,C (DD 89) form.

The DD/FD index-prefix family dispatches through ``_execute_index``, which
handles the indexed load/ALU/POP/PUSH/JP sub-opcodes -- but sub-opcode 0x89
(the ADC A,C form of the 0x80-0xBF ALU block) has no handler yet, so it
raises ``NotImplementedError``.  This file pins DD 89 with one direct
deterministic unit test: PC=0x2000, IX=0x4000, A=0xFF, C=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When DD 89 ADC A,C is implemented it must behave exactly like the plain 0x89
ADC A,C (the prefix is ignored because neither operand is H/L or (HL)): A
receives 0xFF + 0x01 + carry 1 = 0x101, masked to 0x01, with S=0 Z=0 H=1 C=1
(F=0x11), C and the index register stay untouched, F is written so Q latches
...
T-states (4 for the ignored prefix fetch plus 4 for the base ADC A,C).
"""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed ADC A,C (DD 89) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed ADC A,C is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_89_prefixed_adc_a_c() -> None:
    """DD 89 ADC A,C: A=0x01, F=Q=0x11, C/IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0xFF
    ...
    tstates = _run(cpu, bytes([0xDD, 0x89]))
