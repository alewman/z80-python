"""Red-first witness for the FD-prefixed ADC A,IYH (FD 8C) form.
...
(the ADC A,IYH form: the H slot of the 0x80-0xBF ALU block remapped by the
FD prefix to the high byte of IY) has no handler yet, so it raises
``NotImplementedError``.  This file pins FD 8C with one direct deterministic
unit test: PC=0x2000, IY=0x0100, A=0xFF, R=0x3E, F=Q=0x01 (carry set),
WZ=0xBEEF.

When FD 8C ADC A,IYH is implemented it must behave exactly like ADC A,H with
the H slot replaced by the high byte of IY: A receives 0xFF + 0x01 + C(1) =
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed ADC A,IYH (FD 8C) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed ADC A,IYH is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_8c_adc_a_iyh() -> None:
    """FD 8C ADC A,IYH: A=0x01, F=Q=0x11, IY/WZ unchanged, PC=0x2002, R=0x40, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xFD, 0x8C]))

    assert cpu.a == 0x01  # 0xFF + IYH (0x01) + C (0x01) wraps to 0x01
