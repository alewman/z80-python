"""Red-first witness for the DD-prefixed ADD A,IXH (DD 84) form.
...
(the ADD A,IXH form: the H slot of the 0x80-0xBF ALU block remapped by the
DD prefix to the high byte of IX) has no handler yet, so it raises
``NotImplementedError``.  This file pins DD 84 with one direct deterministic
unit test: PC=0x2000, IX=0x0100, A=0xFF, R=0x3E, F=Q=0x00, WZ=0xBEEF.

When DD 84 ADD A,IXH is implemented it must behave exactly like ADD A,H with
the H slot replaced by the high byte of IX: A receives 0xFF + 0x01 = 0x00
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed ADD A,IXH (DD 84) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed ADD A,IXH is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_84_add_a_ixh() -> None:
    """DD 84 ADD A,IXH: A=0x00, F=Q=0x51, IX/WZ unchanged, PC=0x2002, R=0x40, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xDD, 0x84]))

    assert cpu.a == 0x00  # 0xFF + IXH (0x01) wraps to 0x00
