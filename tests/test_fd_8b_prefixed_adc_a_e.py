"""Red-first witness for the FD-prefixed ADC A,E (FD 8B) form.
...
raises ``NotImplementedError``.  This file pins FD 8B with one direct
deterministic unit test: PC=0x2000, IY=0x5000, A=0xFF, E=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When FD 8B ADC A,E is implemented it must behave exactly like the plain 0x8B
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed ADC A,E (FD 8B) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed ADC A,E is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_8b_prefixed_adc_a_e() -> None:
    """FD 8B ADC A,E: A=0x01, F=Q=0x11, E/IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0xFF
    ...
    tstates = _run(cpu, bytes([0xFD, 0x8B]))
