"""Red-first witness for the FD-prefixed ADC A,B (FD 88) form.

The DD/FD index-prefix family dispatches through ``_execute_index``, which
handles the indexed load/ALU/POP/PUSH/JP sub-opcodes -- but sub-opcode 0x88
(the ADC A,B form of the 0x80-0xBF ALU block) has no handler yet, so it
raises ``NotImplementedError``.  This file pins FD 88 with one direct
deterministic unit test: PC=0x2000, IY=0x5000, A=0xFF, B=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When FD 88 ADC A,B is implemented it must behave exactly like the plain 0x88
ADC A,B (the prefix is ignored because neither operand is H/L or (HL)): A
receives 0xFF + 0x01 + carry 1 = 0x101, masked to 0x01, with S=0 Z=0 H=1 C=1
(F=0x11), B and the index register stay untouched, F is written so Q latches
...
T-states (4 for the ignored prefix fetch plus 4 for the base ADC A,B).
"""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed ADC A,B (FD 88) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed ADC A,B is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_88_prefixed_adc_a_b() -> None:
    """FD 88 ADC A,B: A=0x01, F=Q=0x11, B/IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0xFF
    ...
    tstates = _run(cpu, bytes([0xFD, 0x88]))
