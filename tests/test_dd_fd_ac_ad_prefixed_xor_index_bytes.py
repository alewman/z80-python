"""Red-first witnesses for DD/FD XOR A,index-high/index-low forms."""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    try:
        ...
        pytest.fail(f"DD/FD-prefixed XOR index-byte form is not implemented: {exc}")


_CASES = (
    (0xAC, 0x0FF3, 0xFF, 0xAC),
    (0xAD, 0x0FF3, 0x03, 0x04),
)


@pytest.mark.parametrize("prefix", (0xDD, 0xFD), ids=("dd", "fd"))
@pytest.mark.parametrize(("opcode", "index_value", "result", "flags"), _CASES)
def test_prefixed_xor_a_index_byte(
    prefix: int, opcode: int, index_value: int, result: int, flags: int
) -> None:
    """DD/FD AC-AD read only the selected IX/IY high or low byte."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x1234
    cpu.iy = 0x5678
