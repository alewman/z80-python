"""Red-first witness for the FD-prefixed SUB A,C (FD 91) form.
...
raises ``NotImplementedError``.  This file pins FD 91 with one direct
deterministic unit test: PC=0x2000, IY=0x5000, A=0x00, C=0x01, R=0x3E,
F=Q=0x00, WZ=0xBEEF.

When FD 91 SUB A,C is implemented it must behave exactly like the plain 0x91
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed SUB A,C (FD 91) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed SUB A,C is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_91_prefixed_sub_a_c() -> None:
    """FD 91 SUB A,C: A=0xFF, F=Q=0xBB, C/IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0x00
    ...
    tstates = _run(cpu, bytes([0xFD, 0x91]))
