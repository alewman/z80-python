"""Red-first witness for the DD-prefixed SUB A,IXH (DD 94) form.
...
(the SUB A,IXH form of the 0x80-0xBF ALU block) has no handler yet, so it
raises ``NotImplementedError``.  This file pins DD 94 with one direct
deterministic unit test: PC=0x2000, IX=0x0102, A=0x00, R=0x3E, F=Q=0x00,
WZ=0xBEEF.

When DD 94 SUB A,IXH is implemented it must subtract the high byte of IX
...
T-states (4 for the prefix fetch plus 4 for the base SUB A,IXH).
"""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed SUB A,IXH (DD 94) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed SUB A,IXH is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_94_prefixed_sub_a_ixh() -> None:
    """DD 94 SUB A,IXH: A=0xFF, F=Q=0xBB, IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xDD, 0x94]))
