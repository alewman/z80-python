"""Red-first witness for the DD-prefixed SBC A,C (DD 99) form.
...
raises ``NotImplementedError``.  This file pins DD 99 with one direct
deterministic unit test: PC=0x2000, IX=0x4000, A=0x00, C=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When DD 99 SBC A,C is implemented it must behave exactly like the plain 0x99
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed SBC A,C (DD 99) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed SBC A,C is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_99_prefixed_sbc_a_c() -> None:
    """DD 99 SBC A,C: A=0xFE, F=Q=0xBB, C/IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0x00
    ...
    tstates = _run(cpu, bytes([0xDD, 0x99]))
