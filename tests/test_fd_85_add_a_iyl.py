"""Red-first witness for the FD-prefixed ADD A,IYL (FD 85) form.
...
(the ADD A,IYL form: the L slot of the 0x80-0xBF ALU block remapped by the
FD prefix to the low byte of IY) has no handler yet, so it raises
``NotImplementedError``.  This file pins FD 85 with one direct deterministic
unit test: PC=0x2000, IY=0x0001, A=0xFF, R=0x3E, F=Q=0x00, WZ=0xBEEF.

When FD 85 ADD A,IYL is implemented it must behave exactly like ADD A,L with
the L slot replaced by the low byte of IY: A receives 0xFF + 0x01 = 0x00
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed ADD A,IYL (FD 85) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed ADD A,IYL is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_85_add_a_iyl() -> None:
    """FD 85 ADD A,IYL: A=0x00, F=Q=0x51, IY/WZ unchanged, PC=0x2002, R=0x40, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xFD, 0x85]))

    assert cpu.a == 0x00  # 0xFF + IYL (0x01) wraps to 0x00
