"""Red-first witness for the DD-prefixed SUB A,B (DD 90) form.
...
raises ``NotImplementedError``.  This file pins DD 90 with one direct
deterministic unit test: PC=0x2000, IX=0x4000, A=0x00, B=0x01, R=0x3E,
F=Q=0x00, WZ=0xBEEF.

When DD 90 SUB A,B is implemented it must behave exactly like the plain 0x90
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed SUB A,B (DD 90) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed SUB A,B is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_90_prefixed_sub_a_b() -> None:
    """DD 90 SUB A,B: A=0xFF, F=Q=0xBB, B/IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0x00
    ...
    tstates = _run(cpu, bytes([0xDD, 0x90]))
