"""Red-first witness for the DD-prefixed SUB A,IXL (DD 95) form.
...
(the SUB A,IXL form of the 0x80-0xBF ALU block) has no handler yet, so it
raises ``NotImplementedError``.  This file pins DD 95 with one direct
deterministic unit test: PC=0x2000, IX=0x0201, A=0x00, R=0x3E, F=Q=0x00,
WZ=0xBEEF.

When DD 95 SUB A,IXL is implemented it must subtract the low byte of IX
(0x01) from A: A receives 0x00 - 0x01 = 0xFF with S=1 Y=1 H=1 X=1 N=1 C=1
(F=0xBB), the complete IX register stays untouched, F is written so Q latches
...
T-states (4 for the prefix fetch plus 4 for the base SUB A,IXL).
"""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed SUB A,IXL (DD 95) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed SUB A,IXL is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_95_prefixed_sub_a_ixl() -> None:
    """DD 95 SUB A,IXL: A=0xFF, F=Q=0xBB, IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xDD, 0x95]))
