"""Red-first witnesses for DD/FD OR A,index-high/index-low forms."""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    try:
        ...
        pytest.fail(f"DD/FD-prefixed OR index-byte form is not implemented: {exc}")


_CASES = (
    (0xB4, 0x0FF3, 0xFF, 0xAC),
    (0xB5, 0x0FF3, 0xF3, 0xA4),
)


@pytest.mark.parametrize("prefix", (0xDD, 0xFD), ids=("dd", "fd"))
@pytest.mark.parametrize(("opcode", "index_value", "result", "flags"), _CASES)
def test_prefixed_or_a_index_byte(
    prefix: int, opcode: int, index_value: int, result: int, flags: int
) -> None:
    """DD/FD B4-B5 read only the selected IX/IY high or low byte."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x1234
    cpu.iy = 0x5678
