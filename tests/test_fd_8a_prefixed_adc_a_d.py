"""Red-first witness for the FD-prefixed ADC A,D (FD 8A) form.
...
raises ``NotImplementedError``.  This file pins FD 8A with one direct
deterministic unit test: PC=0x2000, IY=0x5000, A=0xFF, D=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When FD 8A ADC A,D is implemented it must behave exactly like the plain 0x8A
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed ADC A,D (FD 8A) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed ADC A,D is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_8a_prefixed_adc_a_d() -> None:
    """FD 8A ADC A,D: A=0x01, F=Q=0x11, D/IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0xFF
    ...
    tstates = _run(cpu, bytes([0xFD, 0x8A]))
