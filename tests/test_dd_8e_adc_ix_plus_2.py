"""Red-first regression coverage for the DD indexed ADC A,(IX+d) form.

confirm-dd-8e-seam established that DD 8E can reuse
``_index_displacement_addr(prefix)`` for the signed IX/IY displacement and WZ
update, and ``_add(x, y, carry)`` through ``_alu_a`` group 1 for ordinary ADC
semantics::

    DD 8E d  ADC A,(IX+d)
    FD 8E d  ADC A,(IY+d)

This file pins the DD 8E form with one direct deterministic unit test: it
places the prefix, opcode and one signed displacement byte in RAM, seeds A,
the index register, the carry flag and the addressed memory byte, executes
exactly one instruction, and asserts the invariants:

* A receives the sum from the existing ADC helper (``_add(A, source, F.C)``).
...
* Q latches the new F value, because ADC writes the F register.
...
DD/FD index-prefix family through ``_execute_index``, but sub-opcode 0x8E is
...
the regression this file pins.  When the indexed ADC A memory form gets
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD/FD indexed ADC A memory form has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            f"DD/FD indexed ADC A,(IX/IY+d) is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_8e_adc_a_mem_at_ix_plus_d() -> None:
    """DD 8E 02 ADC A,(IX+2): 0xF0 + 0x0F + C(1) = 0x00, F/WZ/PC/Q/T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0xF0
    cpu.f.c = 1  # carry input feeds ADC
    displacement = 2  # signed +2
    target = (cpu.ix + displacement) & 0xFFFF
    cpu.write_byte(target, 0x0F)  # addressed memory operand

    tstates = _run(cpu, bytes([0xDD, 0x8E, displacement & 0xFF]))

    # 0xF0 + 0x0F + 1 = 0x100 -> A=0x00, S=0 Z=1 Y=0 H=1 X=0 PV=0 N=0 C=1
    # (F = 0x51).
    assert cpu.a == 0x00
    assert int(cpu.f) == 0x51
    ...
    assert cpu.q == 0x51  # Q latches the new F (ADC writes F)
    assert tstates == 19
