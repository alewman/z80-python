"""Red-first witness for the DD-prefixed SBC A,E (DD 9B) form.
...
raises ``NotImplementedError``.  This file pins DD 9B with one direct
deterministic unit test: PC=0x2000, IX=0x4000, A=0x00, E=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When DD 9B SBC A,E is implemented it must behave exactly like the plain 0x9B
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed SBC A,E (DD 9B) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed SBC A,E is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_9b_prefixed_sbc_a_e() -> None:
    """DD 9B SBC A,E: A=0xFE, F=Q=0xBB, E/IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0x00
    ...
    tstates = _run(cpu, bytes([0xDD, 0x9B]))
