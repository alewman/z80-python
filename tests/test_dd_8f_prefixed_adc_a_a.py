"""Red-first witness for the DD-prefixed ADC A,A (DD 8F) form.
...
raises ``NotImplementedError``.  This file pins DD 8F with one direct
deterministic unit test: PC=0x2000, IX=0x4000, A=0xFF, R=0x3E, F=Q=0x01,
WZ=0xBEEF.

When DD 8F ADC A,A is implemented it must behave exactly like the plain 0x8F
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed ADC A,A (DD 8F) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed ADC A,A is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_8f_prefixed_adc_a_a() -> None:
    """DD 8F ADC A,A: A=0xFF, F=Q=0xB9, IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0xFF
    ...
    tstates = _run(cpu, bytes([0xDD, 0x8F]))
