"""Red-first witness for the DD-prefixed ADC A,IXL (DD 8D) form.
...
(the ADC A,IXL form: the L slot of the 0x80-0xBF ALU block remapped by the
DD prefix to the low byte of IX) has no handler yet, so it raises
``NotImplementedError``.  This file pins DD 8D with one direct deterministic
unit test: PC=0x2000, IX=0x0001, A=0xFF, R=0x3E, F=Q=0x01 (carry set),
WZ=0xBEEF.

When DD 8D ADC A,IXL is implemented it must behave exactly like ADC A,L with
the L slot replaced by the low byte of IX: A receives 0xFF + 0x01 + C(1) =
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed ADC A,IXL (DD 8D) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed ADC A,IXL is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_8d_adc_a_ixl() -> None:
    """DD 8D ADC A,IXL: A=0x01, F=Q=0x11, IX/WZ unchanged, PC=0x2002, R=0x40, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xDD, 0x8D]))

    assert cpu.a == 0x01  # 0xFF + IXL (0x01) + C (0x01) wraps to 0x01
