"""Red-first witness for the FD-prefixed SBC A,IYH (FD 9C) form.
...
(the SBC A,IYH form: the H slot of the 0x80-0xBF ALU block remapped by the
FD prefix to the high byte of IY) has no handler yet, so it raises
``NotImplementedError``.  This file pins FD 9C with one direct deterministic
unit test: PC=0x2000, IY=0x0102 (IYH=0x01 is the source, IYL=0x02 must be
ignored), A=0x00, R=0x3E, F=Q=0x01 (carry in set), WZ=0xBEEF.

When FD 9C SBC A,IYH is implemented it must behave exactly like SBC A,H with
the H slot replaced by the high byte of IY: A receives 0x00 - 0x01 - carry 1
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed SBC A,IYH (FD 9C) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed SBC A,IYH is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_9c_prefixed_sbc_a_iyh() -> None:
    """FD 9C SBC A,IYH: A=0xFE, F=Q=0xBB, IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xFD, 0x9C]))
