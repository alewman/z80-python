"""Red-first regression coverage for the FD indexed ADC A,(IY+d) form.

confirm-fd-8e-seam established that FD 8E can reuse
...
This file pins the FD 8E form with one direct deterministic unit test: it
...
def test_fd_8e_adc_a_mem_at_iy_plus_d() -> None:
    """FD 8E 02 ADC A,(IY+2): 0xF0 + 0x0F + C(1) = 0x00, F/WZ/PC/Q/T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0xF0
    ...
    tstates = _run(cpu, bytes([0xFD, 0x8E, displacement & 0xFF]))
