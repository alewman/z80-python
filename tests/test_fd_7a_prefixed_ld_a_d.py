"""Red-first witness for the FD-prefixed LD A,D (FD 7A) form.
...
``NotImplementedError``.  This file pins FD 7A with one direct deterministic
unit test: PC=0x2000, IY=0x5000, A=0x12, D=0x3C, R=0x3E, F=Q=0x3C, WZ=0xBEEF.

When FD 7A LD A,D is implemented it must behave exactly like the plain 0x7A
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed LD A,D (FD 7A) has no handler in ``_execute_index`` yet, so
    ...
    try:
        ...
        pytest.fail(
            f"FD-prefixed LD A,D is not implemented (opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_7a_prefixed_ld_a_d() -> None:
    """FD 7A LD A,D: A <- D, D/IY/F/WZ unchanged, PC=0x2002, R=0x40, Q=0, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0x12
    ...
    tstates = _run(cpu, bytes([0xFD, 0x7A]))

    assert cpu.a == 0x3C  # LD A,D copies D into A
    assert cpu.d == 0x3C  # source D untouched
    ...
    assert int(cpu.f) == 0x3C  # LD A,D leaves F untouched
