"""Red-first witness for the DD-prefixed ADC A,B (DD 88) form.
...
raises ``NotImplementedError``.  This file pins DD 88 with one direct
deterministic unit test: PC=0x2000, IX=0x4000, A=0xFF, B=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When DD 88 ADC A,B is implemented it must behave exactly like the plain 0x88
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the DD-prefixed ADC A,B (DD 88) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "DD-prefixed ADC A,B is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_dd_88_prefixed_adc_a_b() -> None:
    """DD 88 ADC A,B: A=0x01, F=Q=0x11, B/IX/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.ix = 0x4000
    cpu.a = 0xFF
    ...
    tstates = _run(cpu, bytes([0xDD, 0x88]))
