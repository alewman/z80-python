"""Red-first witness for the FD-prefixed SUB A,IYL (FD 95) form.

The DD/FD index-prefix family dispatches through ``_execute_index``, which
handles the indexed load/ALU/POP/PUSH/JP sub-opcodes -- but sub-opcode 0x95
(the SUB A,IYL form of the 0x80-0xBF ALU block) has no handler yet, so it
raises ``NotImplementedError``.  This file pins FD 95 with one direct
deterministic unit test: PC=0x2000, IY=0x0201, A=0x00, R=0x3E, F=Q=0x00,
WZ=0xBEEF.

When FD 95 SUB A,IYL is implemented it must subtract the low byte of IY
...
T-states (4 for the prefix fetch plus 4 for the base SUB A,IYL).
"""
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed SUB A,IYL (FD 95) has no handler in ``_execute_index``
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed SUB A,IYL is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_95_prefixed_sub_a_iyl() -> None:
    """FD 95 SUB A,IYL: A=0xFF, F=Q=0xBB, IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    ...
    tstates = _run(cpu, bytes([0xFD, 0x95]))
