"""Red-first witness for the FD-prefixed SBC A,D (FD 9A) form.
...
raises ``NotImplementedError``.  This file pins FD 9A with one direct
deterministic unit test: PC=0x2000, IY=0x5000, A=0x00, D=0x01, R=0x3E,
F=Q=0x01 (carry in set), WZ=0xBEEF.

When FD 9A SBC A,D is implemented it must behave exactly like the plain 0x9A
...
def _run(cpu: MemoryCPU, opcode_bytes: bytes) -> int:
    ...
    the FD-prefixed SBC A,D (FD 9A) has no handler in ``_execute_index`` yet,
    ...
    try:
        ...
        pytest.fail(
            "FD-prefixed SBC A,D is not implemented "
            f"(opcode bytes 0x{opcode_bytes.hex(' ')}): {exc}"
        )


def test_fd_9a_prefixed_sbc_a_d() -> None:
    """FD 9A SBC A,D: A=0xFE, F=Q=0xBB, D/IY/WZ unchanged, PC/R advance, 8 T-states."""
    cpu = MemoryCPU()
    cpu.pc = 0x2000
    cpu.iy = 0x5000
    cpu.a = 0x00
    ...
    tstates = _run(cpu, bytes([0xFD, 0x9A]))
